...
imgs = {k: b64(p) for k, p in {
    ...
    "boundary": "runs/boundary_sensitivity.png",
    "convergence": "runs/convergence.png",
    "viscous": "runs/viscous_comparison.png",
}.items()}
...
<strong>指標(v4改定, issue #6)</strong>: 全モデルの駆動距離(9〜42mm)は開口(40〜90mm)の
近接場のため、直接波窓と到達基準の幾何をドライバ中心距離から<strong>開口面の最近点距離</strong>へ
変更(点音源仮定の廃止)。これに伴い到達時間スプレッドの絶対値は旧版より小さくなり
(中心距離基準は大口径モデルを系統的に不利にしていた)、旧版の到達値とは比較不可。
1/r補正付きスペクトル偏差は診断降格、幾何補正なしのスペクトル形状ばらつきを正式指標化。
...
<div class="method"><strong>モデルの位置づけと解析前提の検証(issue #6):</strong>
本レポートの各モデルは公開写真・公称寸法から構成した構造モデルであり、特にDCA型は
「<strong>AMTS的な空間不均質共鳴インサート</strong>」の物理を検証するモデルとして読むこと
— 実売品の音響性能の再現を主張するものではない(剛体・無損失近似。熱粘性損失を持たないため
レゾネータの吸収は過小評価側に倒れる)。全モデルとも駆動距離(9〜42mm)は開口(40〜90mm)の
近接場にあたるため、点音源前提の幾何参照(ドライバ中心レイ・1/r減衰)に依存する指標は
診断扱いとし、空間スペクトル安定性は幾何補正なしのシェイプ指標で評価する。
格子収束(dx 0.5/0.4/0.3mm)・吸収境界(C-PML裁定)・プローブ配置・熱粘性損失の
一括検証は<strong>セクション8</strong>に収録(詳細は issue #6 / ADR 0002)。</div>
...
一貫して低減(10kHz帯 σ5.7→4.8dB、8kHz帯 4.0→3.4dB)。</figcaption></figure>
<table>
<tr><th>指標</th><th>交互配置(理想化)</th><th>実測潰しマップ</th></tr>
<tr><td>カップリングコーム 5-10kHz振れ幅</td><td class='num'>18.4 dB</td><td class='num good'>11.9 dB(スポンジ境界下の4モデル最小; 境界クリーン再測はセクション8)</td></tr>
<tr><td>ピンナ後 波形一致(5-10kHz)</td><td class='num'>0.393</td><td class='num good'>0.511(DX/LCDと同着圏)</td></tr>
<tr><td>ピンナ後 到達スプレッド</td><td class='num'>57 µs</td><td class='num good'>25 µs</td></tr>
<tr><td>入射10kHz帯レベルむら σ</td><td class='num'>5.7 dB</td><td class='num good'>4.8 dB</td></tr>
</table>
...
<tr><td>σ_probe(10kHz帯 − 8kHz帯) [dB]</td><td class='num'>1.2</td><td class='num'>1.2</td><td class='num'>1.1</td><td class='num'><strong>4.1</strong></td><td class='num'>4.6</td></tr>
<tr><td>8k/10kマップ相関 r</td><td class='num'>0.97</td><td class='num'>0.87</td><td class='num'>0.98</td><td class='num'><strong>0.56</strong></td><td class='num'>0.59</td></tr>
</table>
<p>DCAはピンナ各所に届くスペクトルの「形」が他モデルの約4倍ばらつき、実測潰しでもほぼ改善しない。
この約4倍差は<strong>格子収束検証済み</strong>: dx=0.5/0.4/0.3mmの3解像度でDCA 4.1/4.2/3.9dB vs
基準1前後を維持する(セクション8)。
振幅込み波形一致(広帯域窓相関)はこの帯域再配分に鈍感なため0.511まで回復して見えるが、
...
<strong>LCD 0.890 と Z1R 0.883 がほぼ同着首位</strong>、DX 0.803。
16kHz帯を含めた旧集計ではZ1Rが0.783まで沈んでいたが、そのペナルティ
(2稜線干渉の高域ビーミング)は空間聴覚がほぼ使わない帯域に集中していた。
<strong>空間キュー核心の5〜10kHzでは入射はZ1Rが首位(0.904; LCD 0.895、レベルσはLCD 1.9が最良)</strong>
— 大振動板の面が本来効く帯域で効いている。DXは周縁ドロップ(−14.4dB)が重い。
到達整列は開口基準(v4)では全モデル4〜8µs帯に収まり、旧版で見えたDXの時間整列優位は
中心距離基準が大口径モデルを膨らませていた分が大きかったと判明(核心帯ではDX 3µs / Z1R・LCD 4〜5µs)。
<strong>DCA(AMTS実測)は0.814(核心帯0.829)</strong>: AMTSインサートは往路の波面にも作用し、
波形形状一致は最下位(0.884)・入射拡散度は最大(0.071)。核心帯の到達は13µsと4モデルで最も遅れる —
先頭波面は揃うが、後続にレゾネータ散乱の尾を引く、という構図。
...
<strong>LCD</strong>は広帯域で最も均一(σ1.9dB、8kHz帯σ1.4dB)だが、
...
<strong>DCA(AMTS実測)</strong>は広帯域では均一(σ3.2dB)だが、傾斜レゾネータ列が
...
(形状σ 0.8 / 1.4dB) — 同じ波形が僅かなレベル差で届く。
<strong>DX</strong>は上段で±7dBに大きく扇形に開くが、色順(中心→周縁)に整然と並び、
下段ではσ1.4dBまで潰れる — <strong>拡がりはほぼ純粋なレベル差で、スペクトル形状は保存</strong>
されている(小口径ビームの幾何減衰。相似波形が上下に分布する、という描像そのもの)。
<strong>DCA</strong>は上段のσ4.2dBがDX(4.4dB)と同程度に見えるが中身が違う:
色順に並ばず曲線同士が交差し、<strong>下段でもσ2.9dBと束にならない — 形そのものが場所ごとに違う</strong>。
...
<p>レベル(左)ではDXとDCAが同じ位置(勾配0.41〜0.42dB/mm)に来るが、
<strong>形状(右)で二者は分離する</strong>: DXは形状勾配0.13dB/mm・残差0.04dBと
Z1Rと同じ「原点近く」に戻る — DXの急峻さは<strong>形を保った系統的レベル減衰</strong>であり、
近傍同士は常に相似波形。LCDはレベルこそ最も平坦(0.25dB/mm)だが形状の乱れは
Z1R/DXの2倍(残差0.09dB) — 開口短辺の影の縁で形が変わる。
DCAは形状の勾配0.46dB/mm・残差0.14dBとも突出し、<strong>系統的にも局所的にも
...
2つの留保: (1) この序列は<strong>pp1固有</strong> — 8被験者平均では
Z1R 20.1 ≈ DCA 19.9 > DX 17.8 > LCD 16.5 dBとほぼ丸ごと入れ替わる(セクション6)。
(2) 境界クリーン(C-PML)の再測定では<strong>DCAのみ+4dB深く出て(15.9dB)最浅はDXに交代</strong>する
(セクション8) — DCAの数値は吸収境界の残反射がノッチを埋めた分を含んでいた。</figcaption>
</figure>
...
<strong>AMTSの検証(セクション8の数値検証で改訂)</strong>: DCAの前面はピンナ先端からわずか3mmと
4モデル中最接近なのにコームがLCDより10dB浅い — λ/4側枝と散点配置による反射の非コヒーレント化
というAMTSの狙い自体は確認できる。ただし境界クリーン(C-PML)の再測定でDCAは
11.9→<strong>15.9dB</strong>に深まり(他3モデルは±1.5dB以内)、「4モデル最小」はDX(13.6dB)に譲る。
また熱粘性損失(Maa/Crandall粘性ボア損失)を加えた再評価では改善は−0.9dBに留まり、
「実機はさらに良い側」という期待も小さい: <strong>AMTSは最接近を最浅級と引き換えにする設計、
までが正確な結論</strong>。振れ幅がピンナ固有のノッチ構造と同程度に大きい —
...
<strong>DX 0.504 > LCD 0.472 ≈ Z1R 0.460 > DCA 0.439</strong>、
5〜10kHz核心帯では<strong>DX 0.521 ≈ LCD 0.515 ≈ DCA 0.511 > Z1R 0.492</strong> —
核心帯の上位は数字上は三つ巴。ただしDCAの0.511は割引いて読む必要がある:
広帯域窓相関ベースの波形一致は帯域再配分(スペクトル形状の空間ばらつき σ(10k−8k)=4.1dB、
他モデルの約4倍 — 格子収束検証済み、セクション8)に鈍感で、DCA固有の
「場所ごとに違うスペクトル」のコストを十分に罰していない。
方向純度はDX(0.438)とDCA核心帯(0.436)が並び、核心帯の時間整列はZ1R(17µs)首位。
理想化交互配置(核心帯0.393)→実測マップ(0.511)の+0.12は、
前面設計の細部(どのセルを塞ぐか)が総量指標を大きく動かすことの実証。
なお、この振幅込み波形一致は格子解像度に敏感で(dx 0.5→0.3mmでZ1R核心帯0.49→0.74と上振れ、
検証した2モデル間の順位も入れ替わる — セクション8)、僅差の順位は確定的に読まないこと。
解像度・境界に対して頑健なのはレベルマップ・スペクトル形状ばらつき等の空間量の方である。</p>
...
<p>被験者平均で見ると<strong>LCDの核心帯波形一致0.541±0.028が明確な首位</strong>
(pp1比較での「DX/DCAと同着」はpp1がLCDに不利な耳だった)。
<strong>品質の一貫性(被験者間σの小ささ)ではDXが首位</strong>(σ0.020、
到達時間ばらつきも28±8µsと最も一貫)— 小口径ドームの中央集中ビームは
レベルむらという固定費を払う代わりに、誰の耳に対しても同じように振る舞う。
<strong>Z1R(0.487±0.037)は平均最下位</strong>、<strong>DCA(0.514±0.046)はσ最大</strong>:
DCAは耳との相性で0.462〜0.617まで振れる「当たり外れ」の大きいモデルで、
...
(DCA 0.046 / Z1R 0.037)を占める(スピアマンρ=0.6、n=4)。
...
DXは平均こそ一貫(前節σ0.020)だが署名保存は最下位かつ被験者幅0.08〜0.88と最大 —
...
最大タイだが、<strong>核心帯指標の変動幅はDCA・LCDが同率で4モデル最小</strong>(0.028 vs DX 0.056)。
理由は2つ: (1) DCAのコームは相対的に浅いので、動いても外耳道スペクトルへの絶対的影響が小さい。
...
逆に<strong>指標変動が最大なのはDX</strong>(0.056): 小口径ビームの中央集中は距離に敏感で、
軸+2mmで核心帯波形一致が0.521→0.478に落ちる。
...
<h2>8. 数値検証 — 解析前提の妥当性(issue #6)</h2>
<p class="lede">全モデルの駆動距離(9〜42mm)は開口(40〜90mm)の近接場にあたる。点音源前提の
解析(ドライバ中心レイ・1/r減衰)や数値近似(格子・吸収境界・プローブ配置)が結論を作っていないか、
系統的に検証した。指標はv4(開口面基準)へ改定済み(方法欄)。</p>

<h3>吸収境界: C-PML裁定 — DCAのコームだけが補正を受ける</h3>
<figure><img src="data:image/png;base64,{imgs['boundary']}" alt="境界感度">
<figcaption>スポンジ厚・C-PML・格子位相の各条件でのC(f)(pp1、Z1RとDCA)。</figcaption></figure>
<p>C-PML(畳み込みPML、反射〜−114dB — 本レポートのスポンジ30セルは−60dB)を実装し、
同一の格子実現で裁定した。結果: <strong>Z1R/LCD/DXのコームはC-PMLと±1.5dB以内で一致</strong>
(スポンジ境界はほぼ無罪)だが、<strong>DCAだけは11.9→15.9dBと4dB深くなる</strong> —
残反射がAMTSコームのノッチを埋めていた。境界クリーンなpp1コーム順位は
<strong>LCD 20.7 > Z1R 17.4 > DCA 15.9 > DX 13.6dB</strong>で、「DCA最浅」はDXに交代する
(セクション4・結論は補正済み。8被験者パネルのC-PML再基準化は今後の課題)。
副産物として、ドメインサイズ変更が浮動小数点丸めで組立全体を格子に対し半セル動かす
バグを発見・修正した。この「ボクセル化位相」だけでC(f)は1.4〜1.9dB RMS動く —
<strong>C(f)の歯単位の議論のノイズ床</strong>であり、ノッチ衝突判定(±1/12oct)もこの不確かさを持つ。</p>

<h3>格子収束: 空間量は収束、canalスペクトルは未収束</h3>
<figure><img src="data:image/png;base64,{imgs['convergence']}" alt="格子収束">
<figcaption>dx = 0.5 / 0.4 / 0.3mm(セル数 約20M→92M)でのC(f)。</figcaption></figure>
<p>AMTSの最薄形状(列間壁0.57mm)はdx=0.5mmで約1.1セルしかない。dx 0.5/0.4/0.3mmの
収束スタディ(Z1R+DCA実測)の結果は明確に二分された:
<strong>収束しているもの</strong> — 照射レベルマップ(解像度間相関 8k/10kで≥0.98)、
ボクセル化開口率(2%以内)、そして本レポートの核心主張である
<strong>σ(10k−8k) = DCA 4.1/4.2/3.9dB vs Z1R 1.2/0.9/1.1dB(全解像度で約4倍差を維持)</strong>。
<strong>未収束のもの</strong> — canal 1点のC(f)歯構造(振れ幅はZ1R 15.9→19.1、DCA 11.9→17.8dBと
細かくなるほど深く、C-PML補正と同方向)と、振幅込み波形一致の絶対値(Z1R核心帯0.49→0.74)。
つまり<strong>「どの帯域がどこを照らすか」の空間構造は信頼でき、外耳道1点の細かいスペクトル値は
±数dBの数値不確かさを持つ</strong>。階段近似の面積補正(PFFDTD方式)を今後の課題とする。</p>

<h3>熱粘性損失: 効果は実在するが小さい</h3>
<figure><img src="data:image/png;base64,{imgs['viscous']}" alt="粘性損失比較">
<figcaption>DCA実測モデル: 剛体 vs Maa/Crandall粘性ボア損失(7kHz評価、σ≈1e3 s⁻¹)。</figcaption></figure>
<p>これまで「剛体・無損失なのでAMTSの吸収は過小評価(実機はさらに良い)」と留保してきた。
Maa/Crandallの管抵抗を等価流体運動量シンクとして穴内空気セルに実装し
(平面波減衰則exp(−σL/2c)と±5%で一致することを検証済み)、DCAを再評価した結果:
コーム振れ幅11.9→11.0dB、最深ノッチ−5.7→−5.0dB — <strong>改善は約1dBに留まる</strong>。
AMTS吸収の見積もり誤差は粘性の欠落ではなく、上記の境界・解像度の側が支配的だった
(留保の向きが逆転: 剛体値は「保守側」ではなく、境界込みでは楽観側だった)。</p>

<h3>プローブ配置の監査</h3>
<p>既存8被験者×4モデルの公表ランの全プローブ(8,213点)を再検査した。
旧基準(圧力補間ステンシルの空気判定)の違反は<strong>0点</strong>。
新たに速度成分のスタガード補間ステンシルまで要求すると163点(2.0%)が引っかかり
(うちpp3/pp82のLCDでは外耳道基準プローブが該当)、これらの点のintensity系指標
(入射角・拡散度)はバイアスを持つ — ただし<strong>圧力ベースのTF・コーム・波形一致は無傷</strong>。
新基準は今後のラン全てに適用される(v4)。</p>

<h2>9. 結論 — 単独の勝者はいない、目的で選ぶ</h2>
<div class="callout">
<p><strong>LCD型(大型平面磁界)</strong>: 「音圧ごと同じ波形をピンナ全体に」という目標に最も近い
(入射・振幅込み0.890、レベルσ1.9dB、入射角7.9°)。実寸の厚いマグネット列は深いスロットが
コリメータとして働く。代償は開口率32%(写真実測)+厚さ10mm構造の音色影響 —
外耳道でのカップリング変調がpp1では5〜10kHzで<strong>22.2dBと4モデル中最大</strong>
(境界クリーン再測でも20.7dBで最大のまま。最深ノッチ−7.9dB@6.4kHz、
櫛はヘルムホルツ共鳴ではなくピンナ⇄マグネット面の往復干渉が主因)—
と、ピンナ後の方向純度の低さ(拡散度0.576)。
ただし8被験者で均すとコームは平均16.5dBと3モデル中最小(pp1はLCDに最悪の耳だった)で、
<strong>核心帯ポスト・ピンナ0.541±0.028は被験者平均の明確な首位</strong>(セクション6)—
個人差込みで見るとLCDの評価はむしろ上がる。</p>
<p><strong>DX10000CL型(小径フルドーム)</strong>: ピンナ後の方向純度(拡散度0.438)は最強。
旧版の「時間整列最強」はv4開口基準で解消(全モデル4〜8µs帯に収斂 — 旧値は中心距離基準が
大口径モデルを膨らませていた)。小口径ビームの中央集中により周縁の音圧が最も深く落ち(−14.4dB)、
振幅込みでは3位。「音像の輪郭・定位の鋭さ」を最優先し、レベルむらを受容する選択。
<strong>個人差ロバスト性は首位</strong>(核心帯σ0.020、到達28±8µs、セクション6)—
中央集中ビームは誰の耳に対しても同じように振る舞う。
さらに境界クリーン再測では<strong>コーム最浅(13.6dB)がDCAからDXに移る</strong>(セクション8):
小さく浅い前面は、位相設計に頼らず反射そのものが少ない。</p>
<p><strong>DCA型(平面磁界+AMTS、実測潰しマップ)</strong>: メタマテリアルの狙いのうち
「反射の抑制」は方向としては確認できたが、数値検証で減額された —
前面がピンナに最接近(3mm)でコームがLCDより10dB浅いのは事実だが、
境界クリーン再測では<strong>11.9→15.9dBに深まり「4モデル最小」はDX(13.6dB)に譲る</strong>。
頼みの熱粘性損失も加えて−0.9dBに留まる(セクション8):
<strong>最接近と引き換えに最浅「級」を実現する設計</strong>、が正確な評価。
楔構造固有の<strong>「帯域→場所」の系統的マッピングは実測潰しでも残る</strong>:
8kHzは薄端側、10kHzは両端、とピンナの違う場所を違う周波数が照らし、スペクトル形状の
空間ばらつきは他モデルの約4倍(σ(10k−8k)=4.1dB、8k/10kマップ相関0.56 —
格子収束検証済み、セクション8)。
帯域ごとの部位重み付けで成立するピンナキューにとって、これは総量指標に表れないコストであり、
21mm厚スタックの強制する距離42mmと合わせてDCA固有の弱点。
一方、懸念していた装着ズレ感度は<strong>LCDと同率で4モデル最小</strong>と判明(±1〜2mm再装着での
核心帯指標変動0.028、透過マップ並進はr≥0.99で実質不検出 — セクション7):
浅めのコームは「動いても被害が小さい」。
ただし「コーム最浅級」は<strong>pp1固有</strong>だった: 8被験者ではコームは11.7〜37.0dBと
...
<strong>入射品質で単独首位(0.904)</strong>(レベル均一はLCDに僅差で譲る) —
...
前後方向のドライバ角度付けは未検証。数値近似の妥当性(境界・格子・プローブ・損失)は
セクション8で系統検証済み — 未対応の残件は、8被験者パネルのC-PML再基準化と
階段近似の面積補正(いずれもcanal C(f)の絶対値に効く。空間量の結論には影響しない見込み)。</p>
...
<code>fig_reseat</code>。上下ズレは <code>PinnaSpec.offset_y</code>。
数値検証(セクション8)は <code>verify_probes</code> / <code>run_boundary</code>+<code>fig_boundary</code> /
<code>run_convergence</code>+<code>fig_convergence</code> / <code>run_cpml_baseline</code> /
<code>run_viscous</code>+<code>fig_viscous</code>(issue #6、ADR 0002)。
C-PMLは <code>SceneConfig.absorber="cpml"</code>、粘性損失は <code>FilterSpec.viscous_losses</code>。</p>
